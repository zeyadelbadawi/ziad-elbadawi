...
`}for(K++;K<Z&&$.charCodeAt(K)===I;)K++;G=K;continue}K++}return G<Z&&(Y+=$.slice(G,Z)),C0(Y,0,Y.length,Q,V)}function iZ($,J,Z){if($.charCodeAt(J)!==n)return null;let Q=P9($,J,Z,96),V=J+Q;for(;V<Z;){let X=$.indexOf("`",V);if(X<0||X>=Z)return null;let Y=P9($,X,Z,96);if(Y===Q){let G=$.slice(J+Q,X);return G=G.replace(/\n/g," "),G.length>0&&G[0]===" "&&G[G.length-1]===" "&&G.trim().length>0&&(G=G.slice(1,-1)),{node:{type:v.codeInline,text:G},end:X+Y}}V=X+Y}return null}function _$($,J,Z){if($.charCodeAt(J)!==n)return J;let Q=P9($,J,Z,96),V=J+Q;for(;V<Z;){let X=$.indexOf("`",V);if(X<0||X>=Z)return J;let Y=P9($,X,Z,96);if(Y===Q)return X+Y;V=X+Y}return J}function o6($,J,Z){if($.charCodeAt(J)!==Z0)return J;if(J+1<Z&&$.charCodeAt(J+1)===H0){let q=J+2;for(;q<Z&&$.charCodeAt(q)!==c;)q++;return q<Z?q+1:J}if(J+3<Z&&$.charCodeAt(J+1)===k0&&$.charCodeAt(J+2)===u&&$.charCodeAt(J+3)===u){let q=$.indexOf("-->",J+4);return q>=0?q+3:J}let Q=J+1,V=Q;for(;Q<Z;){let q=$.charCodeAt(Q);if(q>=p&&q<=l||q>=K0&&q<=F0||q>=o&&q<=r||q===u)Q++;else break}if(Q===V)return J;let X=$.slice(V,Q).toLowerCase(),Y=!1;for(;Q<Z;){let q=$.charCodeAt(Q);if(q===c){Q++;break}if(q===H0&&Q+1<Z&&$.charCodeAt(Q+1)===c){Q+=2,Y=!0;break}if(q===j9||q===k9){var G=q;for(Q++;Q<Z&&$.charCodeAt(Q)!==G;)Q++;Q<Z&&Q++;continue}if(q===L)return J;Q++}if(Y||d9(X))return Q;let K=1;for(;Q<Z&&K>0;)if($.charCodeAt(Q)===Z0)if(Q+1<Z&&$.charCodeAt(Q+1)===H0){let q=Q+2,U=q;for(;U<Z&&($.charCodeAt(U)>=p&&$.charCodeAt(U)<=l||$.charCodeAt(U)>=K0&&$.charCodeAt(U)<=F0);)U++;if($.slice(q,U).toLowerCase()===X){for(;U<Z&&$.charCodeAt(U)!==c;)U++;if(U<Z&&U++,K--,K===0)return U}Q=U}else{let q=Q+1,U=q;for(;U<Z&&($.charCodeAt(U)>=p&&$.charCodeAt(U)<=l||$.charCodeAt(U)>=K0&&$.charCodeAt(U)<=F0);)U++;$.slice(q,U).toLowerCase()===X&&K++,Q++}else Q++;return Q}function aZ($,J,Z,Q,V){if($.charCodeAt(J)!==B0||J+1>=Z||$.charCodeAt(J+1)!==B0)return null;let X=J+2;for(;X+1<Z;){let Y=$.charCodeAt(X);if(Y===n){let G=_$($,X,Z);if(G>X){X=G;continue}}if(Y===B0&&$.charCodeAt(X+1)===B0){let G=$.slice(J+2,X),K=C0(G,0,G.length,Q,V);return{node:{type:v.textFormatted,tag:"del",children:K},end:X+2}}Y===_0&&X+1<Z&&X++,X++}return null}function nZ($,J,Z,Q,V){if($.charCodeAt(J)!==x0||J+1>=Z||$.charCodeAt(J+1)!==x0)return null;let X=J+2;for(;X+1<Z;){let Y=$.charCodeAt(X);if(Y===n){let G=_$($,X,Z);if(G>X){X=G;continue}}if(Y===x0&&$.charCodeAt(X+1)===x0&&X>J+2){let G=$.slice(J+2,X),K=C0(G,0,G.length,Q,V);return{node:{type:v.textFormatted,tag:"mark",children:K},end:X+2}}Y===_0&&X+1<Z&&X++,X++}return null}function S6($,J,Z){return $<a9?!!(d$($)&u9):O6.test(J[Z])}function k6($,J,Z){return $<a9?!!(d$($)&A9):H6.test(J[Z])}function sZ($,J,Z){var Q=$.charCodeAt(J);if(Q!==O0&&Q!==U0)return null;var V=P9($,J,Z,Q);if(V===0)return null;var X=J>0?$.charCodeAt(J-1):32,Y=J+V<Z?$.charCodeAt(J+V):32,G=k6(X,$,J-1),K=k6(Y,$,J+V),q=J>0?S6(X,$,J-1):!1,U=J+V<Z?S6(Y,$,J+V):!1,F=!K&&(!U||G||q),W=!G&&(!q||K||U),z,M;return Q===O0?(z=F,M=W):(z=F&&(!W||q),M=W&&(!F||U)),{len:V,canOpen:z,canClose:M}}function tZ($,J,Z,Q){if(J.length!==0){for(var V=[],X=0;X<12;X++)V[X]=-1;for(var Y=0;Y<J.length;){var G=J[Y];if(!G.active||!G.canClose){Y++;continue}for(var K=G.ch===O0?0:1,q=K*6+G.len%3*2+(G.canOpen?1:0),U=V[q]!==void 0?V[q]:-1,F=-1,W=Y-1;W>U;W--){var z=J[W];if(!(!z.active||z.ch!==G.ch||!z.canOpen)&&!((G.canOpen||z.canClose)&&(z.len+G.len)%3===0&&z.len%3!==0)){F=W;break}}if(F<0){V[q]=Y-1,!G.canOpen&&(G.active=!1),Y++;continue}var M=J[F],D=M.len>=2&&G.len>=2,O=D?2:1;M.len-=O,G.len-=O;var H=$[M.idx],B=$[G.idx];H.text=H.text.slice(0,H.text.length-O),B.text=B.text.slice(O);var j=M.idx+1,y=G.idx,_=$.slice(j,y),A={type:v.textFormatted,tag:D?"strong":"em",children:_};$.splice(j,y-j,A);for(var g=y-j-1,k=0;k<J.length;k++)J[k].idx>M.idx&&(J[k].idx-=g);for(var b=F+1;b<Y;b++)J[b].active=!1;if(M.len===0&&(M.active=!1,H.text==="")){$.splice(M.idx,1);for(var P=0;P<J.length;P++)J[P].idx>M.idx?J[P].idx--:J[P].idx===M.idx&&(J[P].idx=-1)}if(G.len===0){G.active=!1;var T=G.idx;if(B.text===""){$.splice(T,1);for(var w=0;w<J.length;w++)J[w].idx>T?J[w].idx--:J[w].idx===T&&(J[w].idx=-1)}}else continue;Y++}for(var x=0,C=0;C<$.length;C++){var f=$[C];if(f.type===v.text){var E=f;if(E.text==="")continue;if(x>0&&$[x-1].type===v.text){$[x-1].text+=E.text;continue}}$[x++]=f}$.length=x}}function L6($,J,Z,Q,V){let X=$.charCodeAt(J)===k0,Y=X?J+1:J;if($.charCodeAt(Y)!==z0)return null;var G=$.indexOf("]",Y+1);if(G<0||G>=Z)return null;for(var K=Y+1,q=-1,U=1;K<Z&&U>0;){var F=$.charCodeAt(K);if(F===_0&&K+1<Z){K+=2;continue}if(F===n){var W=_$($,K,Z);if(W>K){K=W;continue}}if(F===Z0){var z=r6($,K,Z);if(z){K=z.end;continue}var M=o6($,K,Z);if(M>K){K=M;continue}}F===z0?U++:F===u0&&U--,K++}if(U!==0)return null;var D=K-1,O=$.slice(Y+1,D),H=K<Z?$.charCodeAt(K):0,B=!1;if(H===s0){var j=!0;for(K++;K<Z&&($.charCodeAt(K)===I||$.charCodeAt(K)===L);)K++;var y="",_=K;if(K<Z&&$.charCodeAt(K)===Z0){for(K++,_=K;_<Z&&$.charCodeAt(_)!==c;){if($.charCodeAt(_)===_0&&_+1<Z){_+=2;continue}if($.charCodeAt(_)===L){j=!1;break}_++}j&&(_>=Z||$.charCodeAt(_)!==c)&&(j=!1),j&&(y=$.slice(K,_),_++)}else if(j)if(Q.e!==void 0&&K>=Q.e)j=!1;else{for(var A=0,g=!1;_<Z;){var k=$.charCodeAt(_);if(k===_0&&_+1<Z){_+=2;continue}if(k===s0)A++;else if(k===t0){if(g=!0,A===0)break;A--}else if(k===I||k===L)break;_++}_>=Z&&!g&&(Q.e===void 0||K<Q.e)&&(Q.e=K),y=$.slice(K,_)}if(j){for(K=_;K<Z&&($.charCodeAt(K)===I||$.charCodeAt(K)===L);)K++;var b;if(K<Z){var P=$.charCodeAt(K);if(P===j9||P===k9||P===s0){var T=P===s0?41:P;K++;for(var w=K;K<Z&&$.charCodeAt(K)!==T;)$.charCodeAt(K)===_0&&K+1<Z&&K++,K++;K>=Z?j=!1:(b=$.slice(w,K),K++)}}if(j){for(;K<Z&&($.charCodeAt(K)===I||$.charCodeAt(K)===L);)K++;(K>=Z||$.charCodeAt(K)!==t0)&&(j=!1)}}if(j){K++,y=e9(y),b!==void 0&&(b=n9(e9(b)));var x=V?.sanitizer||p9,C=x(y,X?"img":"a",X?"src":"href"),f=C===null?null:y;if(X){var E=C0(O,0,O.length,Q,V),W0=v$(E);return{node:{type:v.image,target:f,alt:W0,title:b},end:K}}else{var G0=Q.inAnchor;Q.inAnchor=!0;var h=G0?[{type:v.text,text:O}]:C0(O,0,O.length,Q,V);return Q.inAnchor=G0,!Q.inAnchor&&p$(h)?null:{node:{type:v.link,target:f,title:b,children:h},end:K}}}else K=D+1,B=!0}var D0="",V0=K;if(!B&&H===z0){var o0=K+1;V0=o0;for(var m0=!1;V0<Z&&$.charCodeAt(V0)!==u0;){if($.charCodeAt(V0)===_0&&V0+1<Z){V0+=2;continue}if($.charCodeAt(V0)===z0){m0=!0;break}V0++}if(m0||V0>=Z)return null;var M0=$.slice(o0,V0);if(M0.trim())D0=B$(M0);else{if(P6(O))return null;D0=B$(O)}V0=V0+1}else{if(P6(O))return null;D0=B$(O)}var d=Q.refs[D0];if(!d)return null;if(X)return{node:{type:v.image,target:d.target,alt:v$(C0(O,0,O.length,Q,V)),title:d.title},end:V0};var s=Q.inAnchor;Q.inAnchor=!0;var h=s?[{type:v.text,text:O}]:C0(O,0,O.length,Q,V);return Q.inAnchor=s,!Q.inAnchor&&p$(h)?null:{node:{type:v.link,target:d.target,title:d.title,children:h},end:V0}}function r6($,J,Z){if($.charCodeAt(J)!==Z0)return null;for(var Q=J+1;Q<Z;){var V=$.charCodeAt(Q);if(V===c)break;if(V===I||V===L||V===D9||V===Z0)return null;Q++}if(Q>=Z||$.charCodeAt(Q)!==c)return null;var X=$.slice(J+1,Q),Y=X.match(/^([a-zA-Z][a-zA-Z0-9+.-]{1,31}):([^\x00-\x20]*)$/);return Y?{node:{type:v.link,target:X,title:void 0,children:[{type:v.text,text:X}]},end:Q+1}:X.indexOf("@")!==-1&&/^[a-zA-Z0-9.!#$%&'*+\/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$/.test(X)?{node:{type:v.link,target:"mailto:"+X,title:void 0,children:[{type:v.text,text:X}]},end:Q+1}:null}function eZ($,J,Z,Q){if($.charCodeAt(J)!==z0||J+1>=Z||$.charCodeAt(J+1)!==U9)return null;let V=J+2;for(;V<Z&&$.charCodeAt(V)!==u0&&$.charCodeAt(V)!==L;)V++;if(V>=Z||$.charCodeAt(V)!==u0)return null;let X=$.slice(J+2,V);return X?{node:{type:v.footnoteReference,target:"#"+f0(X),text:X},end:V+1}:null}function $J($,J,Z,Q){if(Q.disableBareUrls)return null;var V="",X=!1,Y=$.charCodeAt(J);if(Y===C9||Y===72?J+8<=Z&&$.charCodeAt(J+1)===h9&&$.charCodeAt(J+2)===h9&&$.charCodeAt(J+3)===W$&&($.charCodeAt(J+4)===g$&&$.charCodeAt(J+5)===L0&&$.charCodeAt(J+6)===H0&&$.charCodeAt(J+7)===H0?V="https://":$.charCodeAt(J+4)===L0&&$.charCodeAt(J+5)===H0&&$.charCodeAt(J+6)===H0&&(V="http://")):Y===B9||Y===70?J+6<=Z&&$.charCodeAt(J+1)===h9&&$.charCodeAt(J+2)===W$&&$.charCodeAt(J+3)===L0&&$.charCodeAt(J+4)===H0&&$.charCodeAt(J+5)===H0&&(V="ftp://"):(Y===F9||Y===87)&&J+4<=Z&&$.charCodeAt(J+1)===F9&&$.charCodeAt(J+2)===F9&&$.charCodeAt(J+3)===p0&&(V="www.",X=!0),!V)return null;let G=J+V.length;for(;G<Z;){let A=$.charCodeAt(G);if(A===I||A===L||A===R||A===D9||A===Z0||A===c)break;G++}for(var K=0,q=0,U=J;U<G;U++){var F=$.charCodeAt(U);F===s0?K++:F===t0&&q++}let W=G;for(;W>J+V.length;){let A=$.charCodeAt(W-1);if(A===p0||A===Q6||A===L0||A===k0||A===r9||A===t0||A===O0||A===U0||A===B0){if(A===t0){if(K>=q)break;q--}W--}else if(A===x$){for(var z=W-2;z>J&&($.charCodeAt(z)>=p&&$.charCodeAt(z)<=l||$.charCodeAt(z)>=K0&&$.charCodeAt(z)<=F0||$.charCodeAt(z)>=o&&$.charCodeAt(z)<=r);)z--;z>=J&&$.charCodeAt(z)===L$?W=z:W--}else break}if(W<=J+V.length)return null;var M=J+(X?4:V.length),D=$.indexOf("/",M);if((D<0||D>W)&&(D=W),X&&$.indexOf(".",M)===-1)return null;for(var O=-1,H=-1,B=D-1;B>=M;B--)if($.charCodeAt(B)===p0)if(O<0)O=B;else{H=B;break}for(var j=H>=0?H+1:M,B=j;B<D;B++)if($.charCodeAt(B)===U0)return null;var y=$.slice(J,W),_=X?"http://"+y:y;return{node:{type:v.link,target:_,title:void 0,children:[{type:v.text,text:y}]},end:W}}function ZJ($,J,Z,Q){if(Q.disableBareUrls)return null;for(var V=J,X=V;V<Z;){var Y=$.charCodeAt(V);if(Y>=p&&Y<=l||Y>=K0&&Y<=F0||Y>=o&&Y<=r||Y===p0||Y===k0||Y===Y9||Y===36||Y===V$||Y===L$||Y===k9||Y===O0||Y===M9||Y===H0||Y===x0||Y===r9||Y===U9||Y===U0||Y===n||Y===L9||Y===w0||Y===O$||Y===B0||Y===u)V++;else break}if(V===X||V>=Z||$.charCodeAt(V)!==X$)return null;V++;for(var G=V,K=-1,q=V;V<Z;){var Y=$.charCodeAt(V);if(Y>=p&&Y<=l||Y>=K0&&Y<=F0||Y>=o&&Y<=r)V++;else if((Y===u||Y===U0)&&V>G)V++;else if(Y===p0){if(V===G)break;var U=$.charCodeAt(V-1);if(U===u||U===U0||V-q>63)break;if(V+1<Z){var F=$.charCodeAt(V+1);if(F>=p&&F<=l||F>=K0&&F<=F0||F>=o&&F<=r)K=V,q=V+1,V++;else break}else break}else break}if(V-q>63||K<0)return null;var W=$.charCodeAt(V-1);if(!(W>=p&&W<=l||W>=K0&&W<=F0||W>=o&&W<=r)||V<=K+1)return null;for(var z=-1,M=K-1;M>=G;M--)if($.charCodeAt(M)===p0){z=M;break}for(var D=z>=0?z+1:G,M=D;M<V;M++)if($.charCodeAt(M)===U0)return null;var O=$.slice(J,V);return{node:{type:v.link,target:"mailto:"+O,title:void 0,children:[{type:v.text,text:O}]},end:V}}function p$($){for(var J=0;J<$.length;J++)if($[J].type===v.link||"children"in $[J]&&Array.isArray($[J].children)&&p$($[J].children))return!0;return!1}function v$($){for(var J="",Z=0;Z<$.length;Z++){var Q=$[Z];Q.type===v.text?J+=Q.text:Q.type===v.breakLine?J+=" ":Q.type===v.codeInline?J+=Q.text:"children"in Q&&Array.isArray(Q.children)?J+=v$(Q.children):Q.type===v.image?J+=Q.alt||"":Q.type===v.link&&(J+=v$(Q.children))}return J}function c9($){let J=$.includes("&")?n9($):$;return{type:v.text,text:J}}function JJ($,J,Z,Q,V){if($.charCodeAt(J)!==Z0)return null;var X=J+1;if(X>=Z)return null;var Y=$.charCodeAt(X);if(Y===k0&&X+1<Z&&$.charCodeAt(X+1)===u&&X+2<Z&&$.charCodeAt(X+2)===u){var G=X+3;if(G<Z&&$.charCodeAt(G)===c)return{node:{type:v.htmlComment,text:"",h:!0},end:G+1};if(G+1<Z&&$.charCodeAt(G)===u&&$.charCodeAt(G+1)===c)return{node:{type:v.htmlComment,text:"-",h:!0},end:G+2};var K=$.indexOf("-->",G);return K!==-1&&K<=Z-3?{node:{type:v.htmlComment,text:$.slice(J+4,K),h:!1},end:K+3}:null}if(Y===r9){var q=$.indexOf("?>",X+1);return q!==-1&&q<Z?{node:{type:v.htmlSelfClosing,tag:"?",attrs:{},a:$.slice(J,q+2),b:!1},end:q+2}:null}if(Y===k0&&X+1<Z){var U=$.charCodeAt(X+1);if(U===z0&&$.slice(X+1,X+8)==="[CDATA["){var F=$.indexOf("]]>",X+8);return F!==-1&&F<Z?{node:{type:v.htmlSelfClosing,tag:"![CDATA[",attrs:{},a:$.slice(J,F+3),b:!1},end:F+3}:null}if(U>=p&&U<=l){var W=$.indexOf(">",X+2);return W!==-1&&W<Z?{node:{type:v.htmlSelfClosing,tag:"!"+$.slice(X+1,W),attrs:{},a:$.slice(J,W+1),b:!1},end:W+1}:null}}if(Y===H0){var z=X+1;if(z>=Z)return null;var M=$.charCodeAt(z);if(!(M>=p&&M<=l||M>=K0&&M<=F0))return null;for(z++;z<Z;){var D=$.charCodeAt(z);if(D>=p&&D<=l||D>=K0&&D<=F0||D>=o&&D<=r||D===u)z++;else break}for(;z<Z&&($.charCodeAt(z)===I||$.charCodeAt(z)===R||$.charCodeAt(z)===L);)z++;if(z<Z&&$.charCodeAt(z)===c){var O=$.slice(X+1,z).trim();return{node:{type:v.htmlSelfClosing,tag:O,attrs:{},a:$.slice(J,z+1),b:!0},end:z+1}}return null}if(!(Y>=p&&Y<=l||Y>=K0&&Y<=F0))return null;var H=N9($,J);if(!H)return null;var B=H.tag,j=B.toLowerCase(),y=H.selfClosing;if(y||d9(B))return{node:{type:v.htmlSelfClosing,tag:B,attrs:_9(H.attrs,B,V),a:$.slice(J,H.end),b:!1},end:H.end};var _=G$.has(j),A=u$($.slice(0,Z),H.end,B);if(A===-1)return{node:{type:v.htmlSelfClosing,tag:B,attrs:_9(H.attrs,B,V),a:$.slice(J,H.end),b:!1},end:H.end};var g=pZ($,"</"+j,A),k=$.slice(H.end,g),b=[];if(_)k.trim()&&(b=[{type:v.text,text:k}]);else{var P=k.trim();if(P){var{inAnchor:T,inline:w}=Q;j==="a"&&(Q.inAnchor=!0);var x=P.indexOf(`

`)!==-1||/^#{1,6}\s/.test(P);x?(Q.inline=!1,b=I9(P,Q,V)):b=C0(P,0,P.length,Q,V),Q.inAnchor=T,Q.inline=w}}return{node:{type:v.htmlBlock,tag:B,attrs:_9(H.attrs,B,V),f:H.rawAttrs,children:b,a:void 0,text:k,d:!1,b:!1},end:A}}var QJ=200,t9=0;function C0($,J,Z,Q,V){if(t9++,t9>QJ)return t9--,[{type:v.text,text:$.slice(J,Z)}];var X=Q.e;Q.e=void 0;let Y=Q;if(V.optimizeForStreaming){let i0=function(S,S0){for(var Y0=1,a0=S0+1;a0<S.length;a0++){var z9=S.charCodeAt(a0);if(z9===z0)Y0++;else if(z9===u0&&(Y0--,Y0===0))return a0}return-1},N=$.slice(J,Z),a=N;for(var G=0,K=0,q=0,U=0,F=0,W=-1,z=-1,M=-1,D=-1,O=-1,H=0;H<N.length;H++){var B=N.charCodeAt(H);B===O0?H+1<N.length&&N.charCodeAt(H+1)===O0?(G++,W=H,H++):(K++,z=H):B===U0?H+1<N.length&&N.charCodeAt(H+1)===U0?(q++,M=H,H++):(U++,D=H):B===B0&&H+1<N.length&&N.charCodeAt(H+1)===B0&&(F++,O=H,H++)}var j=[];F%2===1&&O>=0&&j.push([O,2]),q%2===1&&M>=0&&j.push([M,2]),U%2===1&&D>=0&&j.push([D,1]),G%2===1&&W>=0&&j.push([W,2]),K%2===1&&z>=0&&j.push([z,1]),j.sort(function(S,S0){return S0[0]-S[0]});for(var y=0;y<j.length;y++){var _=j[y][0],A=j[y][1];N=N.slice(0,_)+N.slice(_+A)}let v0=0,r0=-1;for(let S=0;S<N.length;S++)N.charCodeAt(S)===n&&(v0++,r0=S);if(v0%2===1&&r0!==-1){let S=!1,S0=-1,Y0=0;for(;Y0<N.length;)N.charCodeAt(Y0)===n&&(S?(S=!1,S0=-1):(S0=Y0,S=!0)),Y0++;S&&S0!==-1&&(N=N.slice(0,S0))}for(var g=!0;g;){g=!1;for(var k=-1,b=-1,P=-1,T=!1,w=0;w<N.length;w++){var x=N.charCodeAt(w);if(x===z0&&(w===0||N.charCodeAt(w-1)!==_0)){var C=w>0&&N.charCodeAt(w-1)===k0,f=C?w-1:w,E=i0(N,w);if(E===-1)k=f,T=C,b=w+1,P=N.length;else{var W0=E+1;if(W0>=N.length)k=f,T=C,b=w+1,P=E;else if(N.charCodeAt(W0)===s0){var G0=N.indexOf(")",W0+1);G0===-1?(k=f,T=C,b=w+1,P=E,w=N.length):w=G0}else if(N.charCodeAt(W0)===z0){var h=N.indexOf("]",W0+1);h===-1?(k=f,T=C,b=w+1,P=E,w=N.length):w=h}else w=E}}}if(k>=0){var D0=T?"":N.slice(b,P);N=N.slice(0,k)+D0,g=!0}}let b0=N.match(/<([A-Z][A-Za-z0-9]*)(?:\s[^>]*)?>([^<]*)$/);if(b0&&b0.index!==void 0){var V0=b0[0].length-b0[2].length,o0=V0>=2&&b0[0].charCodeAt(V0-2)===H0;if(!o0){for(var m0=!1,M0=0,d=0;d<b0.index;d++)N.charCodeAt(d)===n&&M0++;if(m0=M0%2===1,!m0){let S=b0[1];g9(N,"</"+S,0)===-1&&(N=N.replace(/<[A-Z][A-Za-z0-9]*(?:\s[^>]*)?>([^<]*)$/,"$1"))}}}N!==a&&($=$.slice(0,J)+N,Z=J+N.length)}let s=[];var N0=[];let e=J;var I0=V.disableAutoLink||V.disableBareUrls||Y.inAnchor?-1:$.indexOf("@",J);for(I0>=Z&&(I0=-1);J<Z;){let N=$.charCodeAt(J),a=null;if(N===n){if(a=iZ($,J,Z),!a){var A0=P9($,J,Z,n);J+=A0-1}}else if(N===O0||N===U0){var X0=sZ($,J,Z);if(X0){if(X0.canOpen||X0.canClose){J>e&&s.push(c9($.slice(e,J)));var P0=$.slice(J,J+X0.len),$0=c9(P0);N0.push({idx:s.length,ch:N,len:X0.len,canOpen:X0.canOpen,canClose:X0.canClose,active:!0}),s.push($0),J+=X0.len,e=J;continue}J+=X0.len-1}}else if(N===B0)a=aZ($,J,Z,Y,V);else if(N===x0)a=nZ($,J,Z,Y,V);else if(N===z0)J+1<Z&&$.charCodeAt(J+1)===U9&&(a=eZ($,J,Z,Y)),a||(a=L6($,J,Z,Y,V));else if(N===k0&&J+1<Z&&$.charCodeAt(J+1)===z0)a=L6($,J,Z,Y,V);else if(N===Z0)a=r6($,J,Z),!a&&!V.disableParsingRawHTML&&!V.ignoreHTMLBlocks&&(a=JJ($,J,Z,Y,V));else if((N===C9||N===F9||N===B9)&&!Y.inAnchor&&!V.disableAutoLink&&(J===0||$.charCodeAt(J-1)!==Z0)){var q0=J+1<Z?$.charCodeAt(J+1):0;(N===C9&&q0===h9||N===B9&&q0===h9||N===F9&&q0===F9)&&(a=$J($,J,Z,V))}if(!a&&I0>=0&&I0-J<=64&&!Y.inAnchor&&!V.disableAutoLink&&!V.disableBareUrls&&(N>=p&&N<=l||N>=K0&&N<=F0||N>=o&&N<=r)&&(a=ZJ($,J,Z,V),!a&&J>=I0&&(I0=$.indexOf("@",J+1),I0>=Z&&(I0=-1))),N===H$){J>e&&s.push(c9($.slice(e,J))),s.push({type:v.breakLine}),J++,e=J;continue}if(a)J>e&&s.push(c9($.slice(e,J))),s.push(a.node),J=a.end,e=J;else{if(N===_0&&J+1<Z){let v0=$.charCodeAt(J+1);if(d$(v0)&u9){J>e&&s.push(c9($.slice(e,J))),s.push(c9($[J+1])),J+=2,e=J;continue}}if(J++,I0<0||I0-J>64)for(;J<Z&&!(I0>=0&&I0-J<=64);){var m=$.charCodeAt(J);if(m<a9&&!x6[m])J++;else break}}}if(Z>e){let N=$.slice(e,Z);N&&s.push(c9(N))}return N0.length>0&&tZ(s,N0,Q,V),Q.e=X,t9--,s}var VJ=500;function I9($,J,Z){var Q=J.i||0;if(Q>VJ)return[{type:v.text,text:$}];J.i=Q+1;let V=J;if(Z.optimizeForStreaming&&!J.j){var X=$.length;X>0&&$.charCodeAt(X-1)===L&&X--;for(var Y=X;Y>0&&$.charCodeAt(Y-1)!==L;)Y--;if(Y>0&&$.charCodeAt(Y)===w0){for(var G=!0,K=Y;K<X;K++){var q=$.charCodeAt(K);if(q!==I&&q!==R&&q!==u&&q!==L0&&q!==w0){G=!1;break}}if(G){for(var U=Y-1;U>0&&$.charCodeAt(U-1)!==L;)U--;$.charCodeAt(U)===w0&&($=$.slice(0,U).trimEnd())}}var F=$.trim(),W=F.lastIndexOf(`
`),z=W===-1?F:F.slice(W+1);if(z.length>0&&z.charCodeAt(0)===w0){for(var M=!1,D=!1,O=1;O<z.length;O++){var H=z.charCodeAt(O);H===w0&&(M=!0),H===u&&(D=!0)}M&&D&&($=W===-1?"":$.slice(0,$.lastIndexOf(z)).trimEnd())}for(var B=-1,j=-1,y=-1,_=$.length-1;_>=0;_--)if($.charCodeAt(_)===Z0){var A=_+1<$.length?$.charCodeAt(_+1):0;if(A>=p&&A<=l||A>=K0&&A<=F0){for(var g=_+2;g<$.length;){var k=$.charCodeAt(g);if(k>=p&&k<=l||k>=K0&&k<=F0||k>=o&&k<=r)g++;else break}for(var b=g;b<$.length&&$.charCodeAt(b)!==c;)b++;if(b<$.length&&$.charCodeAt(b-1)!==H0){for(var P=!1,T=b+1;T<$.length;T++)if($.charCodeAt(T)===Z0){P=!0;break}P||(B=_,j=g,y=b+1)}}break}if(B>=0){for(var w=0,x=0;x<B;x++)$.charCodeAt(x)===n&&w++;if(w%2===0){var C=$.slice(B+1,j);g9($,"</"+C,0)===-1&&($=$.slice(0,B)+$.slice(y))}}for(var f=$.length;f>0&&$.charCodeAt(f-1)===L;)f--;if(f>0){for(var E=f;E>0&&$.charCodeAt(E-1)!==L;)E--;for(var W0=E,G0=0;W0<f&&$.charCodeAt(W0)===I&&G0<3;)W0++,G0++;if(E>0&&W0<f&&fZ($,W0,f)){for(var h=E-1,D0=h;D0>0&&$.charCodeAt(D0-1)!==L;)D0--;d0($,D0,h)||($=$.slice(0,E).trimEnd())}}var V0=$.length;if(V0>0){for(var o0=$.lastIndexOf(`
`),m0=o0===-1?0:o0+1,M0=V0,d=m0,s=0;d<M0&&$.charCodeAt(d)===I&&s<3;)d++,s++;if(d<M0){var N0=$.charCodeAt(d),e=!1;if(N0===O0||N0===u||N0===M9){var I0=d+1;if(I0>=M0||$.charCodeAt(I0)===I||$.charCodeAt(I0)===R){for(var A0=I0;A0<M0&&($.charCodeAt(A0)===I||$.charCodeAt(A0)===R);)A0++;A0>=M0&&(e=!0)}}else if(N0>=o&&N0<=r){for(var X0=d;X0<M0&&$.charCodeAt(X0)>=o&&$.charCodeAt(X0)<=r;)X0++;if(X0<M0&&($.charCodeAt(X0)===p0||$.charCodeAt(X0)===t0)){var P0=X0+1;if(P0>=M0||$.charCodeAt(P0)===I||$.charCodeAt(P0)===R){for(var $0=P0;$0<M0&&($.charCodeAt($0)===I||$.charCodeAt($0)===R);)$0++;$0>=M0&&(e=!0)}}}e&&($=$.slice(0,m0).trimEnd())}}}if(J.inline)return C0($,0,$.length,J,Z);let q0=[],m=0,N=$.length;if(m===0&&!Z.disableFrontmatter&&$.startsWith("---")){let S=F6($);if(S&&S.hasValidYaml){if(Z.preserveFrontmatter!==!1){let S0=$.slice(0,S.endPos).trimEnd();q0.push({type:v.frontmatter,text:S0})}m=S.endPos}}for(;m<N;){for(var a=$.indexOf(`
`,m),v0=a<0?N:a;m<N&&d0($,m,v0);)m=v0<N?v0+1:v0,m<N&&(a=$.indexOf(`
`,m),v0=a<0?N:a);if(m>=N)break;$.charCodeAt(m),j0($,m,v0);let S=null;if(m6=$,u6=m,p6=v0,$.charCodeAt(m)!==i9&&J0>=4&&!J.inHTML)S=hZ($,m);else if($.charCodeAt(m)!==i9){let S0=m+t,Y0=$.charCodeAt(S0);Y0===Y9?S=d6($,m,J,Z):Y0===c?S=mZ($,m,J,Z):Y0===n||Y0===B0?S=CZ($,m,J):Y0===u||Y0===O0||Y0===U0?(S=x9($,m),!S&&(S=w6($,m,J,Z))):Y0===M9||Y0>=o&&Y0<=r?S=w6($,m,J,Z):Y0===Z0?S=l6($,m,J,Z):Y0===w0?S=y6($,m,J,Z):Y0===z0&&(S=cZ($,m,J))}if(!S){for(var r0=!1,i0=m;i0<v0;i0++)if($.charCodeAt(i0)===w0){r0=!0;break}r0&&(S=y6($,m,J,Z))}if(S||(S=oZ($,m,J,Z)),S)S.node.type!==v.refCollection&&q0.push(S.node),m=S.end;else{var b0=$.indexOf(`
`,m);m=b0<0?N:b0+1}}return J.i=Q,q0}function c$($,J){t9=0,$.charCodeAt(0)===65279&&($=$.slice(1)),$=T$($);let Z={inline:!1,inAnchor:!1,inHTML:!1,inList:!1,inBlockQuote:!1,refs:{}},Q={...J,slugify:J?.slugify?X=>J.slugify(X,f0):f0,sanitizer:J?.sanitizer||p9,tagfilter:J?.tagfilter!==!1};Z.j=C6($,Z.refs,Q);let V=I9($,Z,Q);return f$(Z.refs)?[{type:v.refCollection,refs:Z.refs},...V]:V}function i6($,J,Z,Q){var V=N9($,J);return V?{tagName:V.tag,tagLower:V.tag.toLowerCase(),attrs:V.rawAttrs,whitespaceBeforeAttrs:V.whitespaceBeforeAttrs,isSelfClosing:V.selfClosing,hasSpaceBeforeSlash:V.hasSpaceBeforeSlash,isClosing:V.isClosing,hasNewline:V.whitespaceBeforeAttrs.includes(`
`)||V.rawAttrs.includes(`
`),endPos:V.end}:null}function N$($,J,Z){t9=0,$=T$($),!J.refs&&(J.refs={}),J.j=C6($,J.refs,Z);let Q=I9($,J,Z);return f$(J.refs)?[{type:v.refCollection,refs:J.refs},...Q]:Q}var GJ=/^\n+/,qJ=typeof document<"u",$9=$=>Array.isArray($)?$:[$],a6=$=>typeof $=="function"||typeof $=="object"&&$!==null&&"render"in $;function n6($){if(!$)return"";let J="";for(let[Z,Q]of Object.entries($))Q===!0?J+=` ${Z}`:Q!=null&&Q!==!1&&(J+=` ${Z}="${String(Q)}"`);return J}function UJ($,J,Z,Q,V,X,Y,G){switch($.type){case v.blockQuote:{let K={},q=$.children;return $.alert&&(K.class="markdown-alert-"+X($.alert.toLowerCase(),f0),q=[D6($.alert),...q]),Q("blockquote",K,...$9(J(q,Z)))}case v.breakLine:return Q("br",{});case v.breakThematic:return Q("hr",{});case v.frontmatter:return G.preserveFrontmatter?Q("pre",{},$.text):null;case v.codeBlock:{let K=$.lang?n9($.lang):"",q={...$.attrs||{},class:K?`language-${K} lang-${K}`:""};return Q("pre",{},Q("code",q,$.text))}case v.codeInline:return Q("code",{},$.text);case v.footnoteReference:return Q("a",{href:V($.target,"a","href")},Q("sup",{},$.text));case v.gfmTask:return Q("input",{checked:$.completed,readOnly:!0,type:"checkbox"});case v.heading:return Q(`h${$.level}`,{id:$.id},...$9(J($.children,Z)));case v.htmlBlock:{let K=$;if(G.tagfilter&&C$(K.tag)){let q=typeof K.a=="string"?K.a:`<${K.tag}${n6(K.attrs)}>`;return Q("span",{},q)}if(K.a&&K.d){let z=function(j){if((j.type===v.htmlSelfClosing||j.type===v.htmlBlock)&&j.b)return[];if(j.type===v.paragraph){let y=j.children;return y?y.flatMap(z):[]}return j.type===v.text?j.text?.trim()?[j]:[]:j.type===v.htmlBlock&&j.children?[{...j,children:j.children?.flatMap(z)}]:[j]},q=K.tag.toLowerCase(),U=T6(q),F=/<[a-z][^>]{0,100}>/i.test(K.a);if(U){let j=K.a.replace(new RegExp("\\s*</"+q+">\\s*$","i"),"");return G.tagfilter&&(j=h$(j)),F?Q($.tag,{...$.attrs,innerHTML:j}):Q($.tag,{...$.attrs},j)}if(/<\/?pre\b/i.test(K.a)){let j=G.tagfilter?h$(K.a):K.a;return Q($.tag,{...$.attrs,innerHTML:j})}if(K.children&&K.children.length>0)return Q($.tag,{...$.attrs},J(K.children.flatMap(z),Z));let M={slugify:j=>X(j,f0),sanitizer:V,tagfilter:!0},D=K.a.replace(/>\s+</g,"><").replace(/\n+/g," ").trim();if(new RegExp(`^<${K.tag}(\\s[^>]*)?>(\\s*</${K.tag}>)?$`,"i").test(D))return Q($.tag,{...$.attrs});let H=N$(D,{inline:!1,refs:Y,inHTML:!1},M),B=J(H.flatMap(z),Z);return Q($.tag,{...$.attrs},...$9(B))}return d9($.tag)?Q($.tag,{...$.attrs}):Q($.tag,{...$.attrs},...$.children?$9(J($.children,Z)):[])}case v.htmlSelfClosing:{let K=$;if(G.tagfilter&&C$(K.tag)){let q=typeof K.a=="string"?K.a:`<${K.tag}${n6(K.attrs)} />`;return Q("span",{},q)}return Q(K.tag,{...K.attrs})}case v.image:return Q("img",{alt:$.alt&&$.alt.length>0?$.alt:void 0,title:$.title||void 0,src:V($.target,"img","src")});case v.link:{let K={};return $.target!=null&&(K.href=j6($.target)),$.title&&(K.title=$.title),Q("a",K,...$9(J($.children,Z)))}case v.table:{let K=$;return Q("table",{},Q("thead",{},Q("tr",{},...K.header.map(function(U,F){return Q("th",{style:K.align[F]==null?{}:{textAlign:K.align[F]}},...$9(J(U,Z)))}))),K.cells.length>0?Q("tbody",{},...K.cells.map(function(U,F){return Q("tr",{},...U.map(function(z,M){return Q("td",{style:K.align[M]==null?{}:{textAlign:K.align[M]}},...$9(J(z,Z)))}))})):null)}case v.text:return $.text;case v.textFormatted:return Q($.tag,{},...$9(J($.children,Z)));case v.orderedList:case v.unorderedList:{let K=$.type===v.orderedList?"ol":"ul";return Q(K,{start:$.type===v.orderedList?$.start:void 0},...$.items.map(function(U,F){return Q("li",{},...$9(J(U,Z)))}))}case v.paragraph:return Q("p",{},...$9(J($.children,Z)));case v.ref:return null;default:return null}}var FJ=($,J,Z,Q,V,X)=>{let Y=(q,U,F)=>{let W=()=>UJ(q,z=>U($9(z)),F,J,Z,Q,V,X);return $?$(W,q,U,F):W()},G=q=>q.map(U=>"text"in U?U.text:""),K=(q,U={})=>{let F=(U.renderDepth||0)+1;if(F>2500)return G(q);let W=[],z=!1;for(let M=0;M<q.length;M++){let D=Y(q[M],K,{...U,renderDepth:F}),O=typeof D=="string";if(O&&z)W[W.length-1]+=D;else if(D!==null)if(Array.isArray(D))for(let H=0;H<D.length;H++)W.push(D[H]);else W.push(D);z=O}return W};return K},zJ=($,J)=>{let Z=s9(J,$,void 0);if(!Z)return $;if(a6(Z))return Z;let Q=s9(J,`${$}.component`,$);return a6(Q),Q};function t6($,J){let Z={...J||{}};Z.overrides=Z.overrides||{};let Q=Z.slugify||f0,V=Z.sanitizer||p9,X=O=>WJ(O,{...Z,wrapper:null});function Y(O,H,...B){if(!qJ){let y=B.length===0?void 0:B.length===1?B[0]:B,_=y===void 0?H:{...H,children:y};return typeof O=="function"?O(_):{t:O,p:_}}let j=s6(O,H,...B);return typeof j=="function"?j():j}function G(O,H,...B){let j=s9(Z.overrides,`${O}.props`,{}),y=H||{};for(let k in y){let b=y[k];if(typeof b=="string"&&b.length>0&&b[0]==="<"&&(E6.test(b)||g6.test(b)||i6(b,0))){let P=X(b.trim());y[k]=k==="innerHTML"&&Array.isArray(P)?P[0]:P}}let _=zJ(O,Z.overrides),A=M6(y?.class,j.class||void 0)||void 0,g={...y,...j};return A&&(g.class=A),y.innerHTML&&j.innerHTML===void 0&&(g.innerHTML=y.innerHTML,delete y.innerHTML),Y(_,g,...B)}let K=Z.createElement||((O,H,...B)=>Y(O,H||{},...B)),q={slugify:O=>Q(O,f0),sanitizer:V,tagfilter:Z.tagfilter!==!1,disableAutoLink:Z.disableAutoLink,disableParsingRawHTML:Z.disableParsingRawHTML,enforceAtxHeadings:Z.enforceAtxHeadings,forceBlock:Z.forceBlock,forceInline:Z.forceInline,preserveFrontmatter:Z.preserveFrontmatter},U=$[0]&&$[0].type===v.refCollection?$[0].refs:{},F=FJ(Z.renderRule,G,V,Q,U,Z),W=F($,{inline:Z.forceInline,refs:U}),z=B6(U);if(z.length&&W.push(G("footer",{},...z.map(function(H){let B=H.identifier.slice(1),j=N$(H.footnote,{inline:!0,refs:U},q),y=F(j,{inline:!0,refs:U});return G("div",{id:Q(B,f0)},B+": ",...$9(y))}))),Z.wrapper===null)return W.length===0?null:W.length===1?W[0]:W;let M=Z.wrapper||(Z.forceInline?"span":"div"),D;if(W.length>1||Z.forceWrapper)D=W;else return W.length===1?W[0]:null;return K(M,{...Z.wrapperProps},D)}function WJ($="",J={}){let Z={...J||{}};Z.overrides=Z.overrides||{};let Q=Z.slugify||f0,V=Z.sanitizer||p9;function X(K){let q=Z.forceInline||!Z.forceBlock&&!z6.test(K),U={slugify:z=>Q(z,f0),sanitizer:V,tagfilter:Z.tagfilter!==!1,disableAutoLink:Z.disableAutoLink,disableParsingRawHTML:Z.disableParsingRawHTML,enforceAtxHeadings:Z.enforceAtxHeadings,forceBlock:Z.forceBlock,forceInline:q,preserveFrontmatter:Z.preserveFrontmatter},F=K;if(!q){let z=F.length;for(;z>0&&(F[z-1]===`
`||F[z-1]==="\r");)z--;F=F.slice(0,z),F=`${F.replace(GJ,"")}

`}let W=N$(q?K:F,{inline:q,refs:Y},U);return t6(W,{...Z,forceInline:q})}let Y={};return X($)}var e6=YJ(void 0);function OJ($,J,...Z){if(typeof document>"u"){let V=Z.length===0?void 0:Z.length===1?Z[0]:Z,X=V===void 0?J:{...J,children:V};return typeof $=="function"?$(X):{t:$,p:X}}let Q=s6($,J,...Z);return typeof Q=="function"?Q():Q}var pJ=$=>{let J={value:$.options};return OJ(e6.Provider,J,$.children)},HJ=$=>{let J=KJ(e6),Z=I$(()=>({...J,...$.options})),Q=I$(()=>{let Y=typeof $.children=="function"?$.children():$.children;return Y??""}),V=I$(()=>c$(Q(),Z()));return I$(()=>t6(V(),{...Z(),wrapperProps:{...Z()?.wrapperProps,...Object.fromEntries(Object.entries($).filter(([Y])=>Y!=="children"&&Y!=="options"))}}))()},rJ=HJ;export{f0 as slugify,p9 as sanitizer,c$ as parser,rJ as default,WJ as compiler,t6 as astToJSX,v as RuleType,pJ as MarkdownProvider,e6 as MarkdownContext,HJ as Markdown};
